...
# MenuItem: Models each Menu Item.
class MenuItem:
...
# The MoneyMachine class models the payment processing system.
class MoneyMachine:
    """Models the payment processing system."""
    CURRENCY = "$"
    COIN_VALUES = {
        "quarters": 0.25,
        "dimes": 0.10,
...
while is_on:
    ...
    if choice == "off":
        ...
        if drink:
            # Check if we have resources AND if the payment is successful
            if coffee_maker.is_resource_sufficient(drink):
                payment_made = money_machine.process_coins()
